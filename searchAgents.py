...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x+dx), int(y+dy)
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in cornersNotVisited:
                    cornersNotVisited.remove((nextx, nexty))
                successor = (((nextx, nexty), cornersNotVisited), action, 1)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    problem.heuristicInfo['food'] = foodGrid.asList()
    totalCost = 0
    curPoint = position
    while problem.heuristicInfo['food']:
        # h(n)=max({d(n,f)|f is food not eaten})
        heuristic_cost, food = \
            max([(util.manhattanDistance(curPoint, food), food) for food in problem.heuristicInfo['food']])
        problem.heuristicInfo['food'].remove(food)
        curPoint = food
        totalCost += heuristic_cost
